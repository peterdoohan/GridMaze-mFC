...
param_dict = {
    ...
    "highest": [13, 12],
    "skip_IBL": [9, 8],
}  # default parameters when skipping preprocessing
...
def submit_jobs(import_path="SpikeSorting", jobs_folder=rep.JOBS_FOLDER, python_path="."):
    ...
    for subfolder in param_dict.keys():
        ...
        for each_session in range(len(sample_paths_df)):
            ...
            script_path = rep.get_ephys_preprocessing_SLURM_script(
                ephys_info,
        ...
        for each_subject in os.listdir(spikesorting_path):
            for each_session in os.listdir((spikesorting_path / each_subject)):
                ...
                for each_probe in probes:
                    ...
                    subject_paths_df = first_last_df.query(f'subject_ID == "{each_subject}"').datetime.apply(
                    ...
                    try:
                        ks_labels = sps.pd.read_csv(
                            session_path / "kilosort4" / "sorter_output" / "cluster_KSLabel.tsv", sep="\t"
                        )
                    except:
...
def plot_unit_counts(optim_df):
    ...
    plot_df = optim_df.melt(
        id_vars=["session_path", "ks_params", "subject_ID", "session"],
        value_vars=["n_good", "n_single"],
        var_name="inclusion",
        value_name="count",
    )
...
def plot_qual_metrics_dist(optim_df, single_units_only=False, per_subject=False):
    ...
    big_melt = big_metrics_df.melt(id_vars=identifiers, value_vars=metric_names, var_name="metric")
    ...
    if per_subject == True:
        for each_subject in optim_df["subject_ID"].unique():  #
            ...
            g = sns.FacetGrid(
                subject_df, col="metric", row="session", hue="ks_params", palette="rocket_r", sharey=False, sharex=False
            )
        ...
        g = sns.FacetGrid(
            big_melt, col="metric", row="session", hue="ks_params", palette="rocket_r", sharey=False, sharex=False
        )
        g.map(sns.kdeplot, "value")
        g.add_legend()  #
        if single_units_only == True:
...
def submit_test():
    ...
    # Submit test job
    script_path = rep.get_ephys_preprocessing_SLURM_script(
        ephys_info, kilosort_Ths=kilosort_Ths, spikesort_path=spikesorting_path
    )
    os.system(f"chmod +x {script_path}")
    os.system(f"sbatch {script_path}")
    print(f"Test job submitted for {ephys_info.subject_ID} {ephys_info.datetime.isoformat()}")
