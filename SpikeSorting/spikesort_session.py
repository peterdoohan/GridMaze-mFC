        ...
        if probe_label != None:
            ...
            for i,each_path in enumerate(preprocessed_paths):
                if probe_label in str(each_path):
                    matches.append(i)
            if len(matches)==0:
                raise ValueError(f'Probe label {probe_label} did not match any recording paths (below). See also save_rec_probe() \n {preprocessed_paths} ')
            else:
...
def get_probe_recordings(subject_ID, datetime, ephys_path, 
    ...
    if n_probes == 1: #start with simple case
        ...
        if count_stream_ids(ephys_path) == 1:
            ...
            for i,probe in enumerate(PROBE_LABELS): #renaming probes could happen at preprocessed -> processed data stage.
            ...
            for probe in PROBE_LABELS: 
                preprocessed_paths.append(Path(spikesort_path) / subject_ID / datetime/ f'{probe}')
                probe_rec = se.read_openephys(ephys_path,
                                              stream_id = get_stream_id(ephys_path, probe_label=probe))
                probe_rec = check_rec_probe(probe_rec, subject_ID, probe_label = probe)
                recordings.append(probe_rec)
    else:       
...
def save_rec_probe(subject_ID:str, #required
                        ...
                        probe_label = None, manual_ephys_path = None):
...
def get_first_last_df(min_duration_min=30,
