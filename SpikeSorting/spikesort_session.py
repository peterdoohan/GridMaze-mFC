...
SAMPLING_FREQUENCY = (
    30000  # 30kHz sampling rate used for AP data. This is used to check that the correct data is loaded.
)
...
def preprocess_ephys_session(
    ...
    raw_recs, preprocessed_paths = get_probe_recordings(
        subject_ID, datetime, ephys_path, spikesort_path=spikesort_path, n_probes=n_probes
    )
        ...
        if not (preprocessed_path / "DONE.txt").exists():
            if (
                ...
                preprocessed_rec = pre_kilosort_processing(
                    raw_rec, preprocessed_path, IBL_preprocessing, plot=save_QC_plots  # toggle (True or False)
                )
                ...
                if cache_preprocessed_data:  # this should really always be true.
                    ...
                    if not temp_path.exists():
                        temp_path.mkdir()
                        preprocessed_rec = (
                            preprocessed_rec.save(  # note that saving to binary speeds up kilosorting by 5x
    ...
    for each_shank in range(n_shanks):
        ...
        if plot_outside_brain_channels:
            ...
            sw.plot_traces(
                ...
                return_scaled=True,
                time_range=[500, 600],  # arbitrary time window ~8 min into rec
...
def save_channel_assignment_params(
    subject_ID: str, outside_thresh=0.75, n_neighbours=11, probe_label=None, min_duration_min=20
):
    ...
    first_last_df = get_first_last_df(min_duration_min=min_duration_min)
    ...
    if probe_label == None:
        ...
        n_probes = 2
        assignment_path = assignment_path / probe_label
    ...
    for index in [0, 1]:  # for the first and last recording of a subject
        ...
        raw_recs, preprocessed_paths = get_probe_recordings(
            ephys_info.subject_ID, ephys_info.datetime.isoformat(), ephys_info.ephys_path, n_probes=n_probes
        )
        ...
        if probe_label != None:
            ...
            for i, each_path in enumerate(preprocessed_paths):
                if probe_label in str(each_path):
                    matches.append(i)
            if len(matches) == 0:
                raise ValueError(
                    f"Probe label {probe_label} did not match any recording paths (below). See also save_rec_probe() \n {preprocessed_paths} "
                )
            else:
        ...
        for raw_rec, preprocessed_path in zip(raw_recs, preprocessed_paths):
            ...
            get_channel_assignments(raw_rec, preprocessed_path, outside_thresh, n_neighbours, save_params=True)
...
def save_unitmatch_inputs(raw_rec, preprocessed_rec, preprocessed_path):
    ...
    for half in range(2):
        split_analysers[half].compute("random_spikes", method="uniform", max_spikes_per_unit=500)
    ...
    erd.save_avg_waveforms(
        all_waveforms,
        UM_input_dir.resolve(),  # absolute path for unitmatch
        good_units=0,
...
def save_unitmatch_labels(preprocessed_path):
    ...
    cluster_group_df.to_csv(
        preprocessed_path / "UM_inputs" / "cluster_group.tsv", sep="\t", index=False
    )  # index false is important here.
...
def get_probe_recordings(subject_ID, datetime, ephys_path, spikesort_path=SPIKESORTING_PATH, n_probes=1):
    ...
    if n_probes == 1:  # start with simple case
        ...
        # at this stage assume different probes are named ProbeA, ProbeB.
        ...
        if count_stream_ids(ephys_path) == 1:
            # if there's only one stream_id, assume the probes are recorded in one file and split them:
            for i, probe in enumerate(
                ...
                channel_indexing = np.add(
                    list(range(half_channels)), half_channels * i
                )  # first or second half for A or B.
        ...
        elif count_stream_ids(ephys_path) == 2:
...
def count_stream_ids(ephys_path):
    """Crawls subfolders to find the number of stream_id's.
    Assumes data recorded with OpenEphys and a single record node folder.
    There should be one stream_id for each folder under .../continuous"""
    continuous_dirs = list(Path(ephys_path).glob("**/continuous"))
    # For each continuous folder, count how many subdirectories it contains
    for cont_dir in continuous_dirs:
        subfolder_count = sum(1 for p in cont_dir.iterdir() if p.is_dir())
    return subfolder_count


def get_stream_id(ephys_path, data_type=None, probe_label=None):
    """Flexibly specify stream_id for several usecases.

    Parameters
    ----------
     ephys_path: str()
         path/to/ephys/subject_ID/datetime/ for spike interface to read.
     data_type: str(); 'AP' or 'LFP'
         used when stream_ID's correspond to action potential (AP) versus local field potential (LFP) streams.
     probe_label: str(); 'ProbeA' or 'ProbeB'
         used when stream_ID's correspond to different probes for multi-probe recordings.
         Note that this may not always be the case.

    Returns
    -------
    The correct stream_id for spike-interface open ephys reading.
    Defaults to stream_id = "0".

    Notes
    -----
    Fixes an issue where multiple node recordings duplicated data in subject folders.
    ...
    if (data_type is None) and (probe_label is None):  # meta-level defaulting for edge cases.
        data_type = "AP"
...
def check_rec_probe(raw_rec, subject_ID, probe_label=None):
    ...
    probe_label argument to be passed through when there is more than one probe."""
    ...
    if probe_label is not None:
        probe_params_path = probe_params_path / probe_label
...
def save_rec_probe(
    ...
    try:
        ...
        probe = get_probe(manufacturer=manufacturer, probe_name=probe_name)  # probeinterface function
...
def get_ephys_paths_df():
    ...
    all_ephys_paths = [f for f in all_ephys_paths if f.name != "ignore"]
    ...
    if not (EPHYS_PATH / "ephys_paths_df.tsv").exists():
        ...
        for path in all_ephys_paths:
            ...
            try:
                ...
                rec = se.read_openephys(path, stream_id=stream_id, block_index=0)
...
def get_first_last_df(min_duration_min=30, manual_start_date=MANUAL_START_DATE, manual_end_date=MANUAL_END_DATE):
    """
    ...
    for each_subject in df["subject_ID"].unique():
        subject_df = df.query(
            f'subject_ID == "{each_subject}" and duration_min>{min_duration_min} and spike_interface_readable==True'
        )
