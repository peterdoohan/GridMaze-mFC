...
from devices import Grid_maze_3x3, Rsync
...
v.goal_set = ['A1', 'A2', 'A3', 
              'B1', 'B2', 'B3', 
              'C1', 'C2', 'C3']                             # Set of active goals on the 3x3 maze
...
v.n_errors = 0                                              # Number of incorrect nose pokes (poked port != goal port)
...
#Parameters
v.session_duration = 20                     # Duration of entire session
v.audio_cue_duration = 0.5 * second         # Duration of audio tone during goal cueing 
v.min_ITI_dur = 60                          # Minimum ITI duration (seconds).
v.max_ITI_dur = 120                         # Maximum ITI duration (seconds).
v.reward_consumption_dur = 0.5 * second     # Timer for drinking breaks during reward state
v.reward_duration = 215                     # Time solenoid allows water flow, duration of reward delivery (ms)
...
# for a max duration set by reward_consumption_timer, once this timer has elapsed or mice reach the max_reward_state_dur they move to the ITI
# state. Trial variables are also printed here. 
def reward(event):
    if event == 'entry':
        print('T#:{} E#:{} S:{} Time#:{}'.format(v.n_trials, v.n_errors, v.current_goal, v.trial_end_time - v.trial_start_time))    
