...
def get_SLURM_script(subject, maze_name, day_on_maze, output_type, training_trial_phases):
    """ """
    exp_name = f"{output_type}_decoding_{subject}_{maze_name}_{day_on_maze}_training_{training_trial_phases}"
    script = f"""#!/bin/bash
...
python -c \"from GridMaze.analysis.distance_to_goal import place_decoding as dp; dp.run_session_place_decoding(('{subject}', '{maze_name}', {day_on_maze}), '{output_type}', '{training_trial_phases}')\"
"""
