...
def get_theta_displacement_SLURM_script(
    pcs_from,
    other_fn_params={
        "smooth_SD": 2.5,
    ...
    """Create SLURM script for running nbeGLM experiment."""
    _job_name = f"theta_displacement_{pcs_from}"
    # create SLURM script
    script = f"""#!/bin/bash
#SBATCH --job-name={_job_name}
#SBATCH --output=jobs/theta_mod//out/{_job_name}.out
...
python <<EOF
from GridMaze.analysis.theta_mod import displacement as dis
dis.get_theta_displacement_summary_df(pcs_from='{pcs_from}', **{other_fn_params})
EOF
"""
    script_path = f"jobs/theta_mod/slurm/{_job_name}.sh"
    with open(script_path, "w") as f:
