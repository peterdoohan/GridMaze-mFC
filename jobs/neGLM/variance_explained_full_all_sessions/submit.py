...
def submit_jobs(seed=0, subfolder="variance_explained_full_all_sessions"):
...
def get_model_set_params(seed=0, subfolder="variance_explained_full_all_sessions"):
    model_set_params = []
    all_input_groups = [
        "place_direction",
        "distance_to_goal",
    ...
    for maze_name in ["maze_1", "maze_2", "rooms_maze"]:
        for remove_groups, input_group_kwargs, model_name in [
            ...
            (
                [],
                {"components": ["tower_bridge"]},
                "remove_egocentric_action",
            ),  # remove egocentric action but keep dummy regressor
            (["velocity"], {}, "remove_velocity"),
        ]:
