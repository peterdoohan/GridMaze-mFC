...
class Encoder(torch.nn.Module):
    ...
    def encode(self, x):
        ...
        if self.latent_inputs is not None:  # directly add some inputs to the latents
            if self.combined_frs:
                z = torch.concatenate(
                    [z, x[None, self.latent_inputs, ...] + torch.zeros([len(z), len(self.latent_inputs), x.shape[-1]])],
                    dim=1,
                )
            else:
    ...
    def eval_exp_poisson_cv(
        self, z_train, y_train, z_test, y_test, alphas=10.0 ** np.arange(2, -5, -1), cv=5, alpha=None
    ):
        Tf = len(y_train)
    ...
    def eval_representation(self, x, y, cv=None, return_keep=False, alpha=1e-10, embed=True, trials=None, split=None):
        ...
        if split is not None:
            cv = len(split)  # optionally provide a split
        N, T = y.shape
        ...
        if self.combine_frs:
            if cv is None:
                ...
                for fold in range(cv):
                    test, train = inds[fold], np.concatenate([inds[f] for f in range(cv) if f != fold])
                    scores[keep, fold] = self.eval_comb(
                        z[..., train], y[:, train], z[..., test], y[:, test], alpha=alpha, cv=cv
                    )
        else:
            z = z.T
            for n in range(N):
                ...
                if cv is None:  # no crossvalidation; just test representation on the whole thing
                    ...
                    for fold in range(cv):
                        test, train = inds[fold], np.concatenate([inds[f] for f in range(cv) if f != fold])
                        scores[n, fold] = self.eval_cv_function(
                            z[..., train, :], y_n[train], z[..., test, :], y_n[test], alpha=alpha, cv=cv
                        )
...
# %% copied from additive_multiplicative_regression.py
