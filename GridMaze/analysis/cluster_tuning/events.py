...
def plot_trial_aligned_rates(trial_aligned_rates, goal_stratified=False, smooth_SD=10, ax=None, color="darkgreen"):
    if ax is None:
...
def plot_session_event_aligned_rates(session, goal_stratified=False):
    ...
    for cluster_unique_ID in keep_clusters:
        cluster_event_aligned_rates = event_aligned_rates_df[
            event_aligned_rates_df.cluster_unique_ID == cluster_unique_ID
        ]
