...
def test(
    ...
    model2regressor_classes = {
        "full": ["place_direction", "distance", "goal_by_distance"],
    ...
    for model, regressor_classes in model2regressor_classes.items():
        opt_alphas = opt_population_alpha(
            ...
            goals=goals,
            verbose=True,
        )  # len cluster_unique_IDs
        model2opt_alphas[model] = opt_alphas
...
def opt_population_alpha(
    ...
    """
    get cross-validated optimal regularisation (l2) for every cell, for a single fold /
    set of input regressors
    """
    cluster_unique_IDs = input_data.spike_count.columns.values
    train_df, test_df = folds._get_test_train_dfs(input_data, fold_df, training_trial_phases=False)
    X_train, X_test, Y_train, Y_test = (
    ...
    def _find_alpha(X_train, y_train, X_test, y_test, tol, max_rounds, patience, verbose=False):
        """
    ...
    for i, cluster_unique_ID in enumerate(cluster_unique_IDs):
        if verbose:
            print(cluster_unique_ID)
        y_train, y_test = Y_train[:, i], Y_test[:, i]  # n_train_samples, n_test_samples
        alpha = _find_alpha(X_train, y_train, X_test, y_test, tol, max_rounds, patience, verbose)
        alphas.append(alpha)
