...
def plot_anatomical_egocentric_action_tuning(
    ...
    for ax in axes:
        ax.invert_yaxis()
        ax.invert_xaxis()
    cbar = f.colorbar(sc, ax=axes[1], label="early --> late \n action tunned")
...
def plot_anatomical_axis_egocentric_action_tuning(
    ...
    for ax, a, n_bins, label in zip(axes, ["x", "y", "z"], n_axis_bins, ["P -> A", "D -> V", "M -> L"]):
        axis_grouped = anat_df.groupby([("voxel", a)]).is_tunned
        total_cells = axis_grouped.count()
        sum_tunned = axis_grouped.sum()
        # downsample voxels
        n_voxels = len(total_cells)
        if n_voxels > n_bins:
            group_n_voxels = n_voxels // n_bins
            _n_groups = np.minimum(np.arange(n_voxels) // group_n_voxels, n_bins - 1)
        ...
        ax.plot(x, y, linewidth=1.5, label=a, color="k")
        ax.set_xlabel(label)
        ax.set_xticks([])
        ax.set_xticklabels([])
        if a == "x":
            ax.set_ylabel("prop. egocentric-action \n tunned")
