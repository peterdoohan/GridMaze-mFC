...
def _test_regularisation(activity_df, validation_folds_df, i_fold=0, inv_alphas=[None, 1e-3, 1e-2, 1e-1, 1]):
    ...
    for inv_alpha in inv_alphas:
        ...
        for i in range(len(timepoints)):
            ...
            train_predictions = logreg.predict(training_activity)
            train_accuracy = (train_predictions == training_y).mean()
            train_perf.append(train_accuracy)
        av_train_perf.append(np.mean(train_perf))
        av_test_perf.append(np.mean(test_perf))
    return av_test_perf, av_train_perf
