...
class Encoder(torch.nn.Module):
    ...
    def decode(self, z, neuron_inds=None):
        ...
        # output matrix and bias for these neurons
    ...
    def eval_representation(self, x, y, cv=None, return_keep=False, alpha=1e-10, embed=True, trials = None, split = None):
        ...
        if embed:
            z = self.encode(x).T.detach().cpu().numpy()  # T x D
        else:  # use the raw inputs
            z = x.T.detach().cpu().numpy()
        ...
        for n in range(N):
            y_n = y[n, :].cpu().numpy()  # target spike counts
            ...
            if cv is None:  # no crossvalidation; just test representation on the whole thing
                ...
                if np.amin([y_n[ind].sum() for ind in inds]) == 0:  # no spikes in one of the splits
                    print("no spikes; excluding clusters. Set return_keep to return indices of included clusters")
                    keep[n] = False
