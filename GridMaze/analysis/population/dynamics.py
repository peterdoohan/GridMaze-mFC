...
def get_population_trial_dynamics(session, stratified_by="goal", n_components=10, smooth_SD=0.5):
    ...
    navigation_rates_df = session.get_navigation_activity_df(type="rates", 
                                                             with_routes = True,
                                                             cluster_kwargs={"single_units": True,
                                                                             "multi_units": True})
    ...
    princple_components_df = pd.DataFrame(princple_components, 
                                          columns=pd.MultiIndex.from_product([["principle_component"],range(1,n_components+1)]))
    ...
    navigation_pcs_df.loc[:,("progress_to_goal", "binned")] = pd.cut(navigation_rates_df.progress_to_goal.path_length, 
                                                                 bins=30, include_lowest=True)
    if stratified_by == "goal":
        population_dynamics_df = navigation_pcs_df.groupby([("goal",""), ("progress_to_goal", "binned")], observed=True).principle_component.mean()
    elif stratified_by == "route":
        population_dynamics_df = navigation_pcs_df.groupby([("route","r"), ("progress_to_goal", "binned")], observed=True).principle_component.mean()
